...
{$myCaseWhereClause} {$myGroupByClause}";
 ...
 AND civicrm_case.is_deleted     = {$cases['case_deleted']}";
  ...
  public static function createCaseRoleActivity($caseId, $relationshipId, $relContactId = NULL, $contactId = NULL) {
    ...
    $query = "
   SELECT  a.display_name as name_a,
           ...
           civicrm_relationship.contact_id_a as id_a,
           civicrm_relationship.contact_id_b as id_b
     FROM  civicrm_relationship_type,  civicrm_relationship
 LEFT JOIN  civicrm_contact b ON b.id = civicrm_relationship.contact_id_b
 LEFT JOIN  civicrm_contact a ON a.id = civicrm_relationship.contact_id_a
    ...
    while ($dao->fetch()) {
      ...
      if ($dao->id_b != $contactId) {
        $caseRelationship = $dao->relation_a_b;
        $assigneeContactName = $dao->name_b;
        $assigneeContactIds[$dao->id_b] = $dao->id_b;
      }
      else {
        $caseRelationship = $dao->relation_b_a;
        $assigneeContactName = $dao->name_a;
        $assigneeContactIds[$dao->id_a] = $dao->id_a;
      }
    }
    ...
    $activityParams = array(
      ...
      'subject' => $caseRelationship . ' : ' . $assigneeContactName,
    ...
    if (!empty($relContactId)) {
      $activityParams['assignee_contact_id'] = $assigneeContactIds;
