...
{if $duplicate == "confirmed"}
  <p>
  {ts}You have already signed.{/ts}
  </p>
{/if}
{if $duplicate == "unconfirmed"}
  <p>{ts}You have already signed but you still <b>need to verify your email address</b>.{/ts}</br> {ts}Please check your email inbox for the confirmation email. If you don't find it, verify if it isn't in your spam folder.{/ts}</p>
{/if}
{if $duplicate}
  <p>{ts}Thank you.{/ts}</p>
  {if $is_share}
