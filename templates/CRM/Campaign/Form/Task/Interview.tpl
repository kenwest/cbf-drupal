    ...
    <table id="voterRecords-{$instanceId}" class="display crm-copy-fields">
      ...
      <tbody>
        ...
        <tr id="row_{$voterId}" class="{cycle values="odd-row,even-row"}" entity_id="{$voterId}">
          {capture assign=voterURL}{crmURL p="civicrm/contact/view" q="reset=1&cid=$voterId"}{/capture}
          {foreach from=$readOnlyFields item=fTitle key=fName}
            {if $fName neq 'contact_type'}
              <td class="name"><a href='{$voterURL}'>{$voterDetails.$voterId.$fName}</a></td>
            {else}
